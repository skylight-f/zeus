...
「扩展管理 → MCP 服务」展示原 Codex 与 Zeus 专属 `config.toml` 中的服务。Codex 会话新建或重新连接时自动读取原配置的 `mcp_servers`，保留停用状态，同名服务以 Zeus 专属配置为准。此兼容仅读取 MCP 配置，不同步原 Codex 的账号、历史或其他设置；MCP 的独立 OAuth 登录仍需在 Zeus 使用的 Codex 环境完成。页面展示配置状态，实际连接情况查看会话运行详情。
