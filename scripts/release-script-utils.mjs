...
export function validateReleaseNotes(markdown, version) {
  const requiredTitle = `# ${distributionAppName} ${version} 更新内容`;
  ...
  if (zeusDistribution.homebrewEnabled && !markdown.includes(`brew upgrade --cask ${zeusDistribution.homebrewTap}/${zeusDistribution.cask}`)) throw new Error('Release notes 缺少 Homebrew 升级命令。');
  if (!markdown.includes(`${distributionArtifactPrefix}-${version}-arm64.dmg`)) throw new Error(`Release notes 缺少版本化 DMG 名称：${distributionArtifactPrefix}-${version}-arm64.dmg。`);
