...
import { zeusDistribution as distribution } from '../packages/shared/src/distribution.ts';
...
if (process.argv.includes('--github-output')) {
  ...
  appendFileSync(process.env.GITHUB_OUTPUT, `homebrew_enabled=${distribution.homebrewEnabled}\nhomebrew_repository=${distribution.homebrewRepository}\n`);
}
...
if (manifestIndex >= 0) {
  ...
  if (process.env.RELEASE_TAG && `v${manifest.version}` !== process.env.RELEASE_TAG) throw new Error('产物版本与发布标签不一致。');
  if (!manifest.artifacts?.length) throw new Error('产物清单为空。');
  for (const artifact of manifest.artifacts) {
    if (!artifact.downloadUrl.startsWith(`https://github.com/${distribution.repository}/releases/download/v${manifest.version}/`)) throw new Error('产物下载来源不属于本次发行版。');
  }
}
...
if (process.argv.includes('--check-version')) {
  ...
  for (const release of pages.flat()) {
    if (release.draft || release.prerelease || release.tag_name === `v${version}` || !/^v\d+\.\d+\.\d+$/u.test(release.tag_name)) continue;
    const current = version.split('.').map(Number);
    const published = release.tag_name.slice(1).split('.').map(Number);
